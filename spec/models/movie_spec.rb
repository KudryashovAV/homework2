...
describe Movie do
  ...
  context "validates" do
    ...
    let(:movie3) {Movie.create(title: '', rating: 'G', release_date: '1936-05-05', description: 'this is a very interesting movie')}
    let(:movie4) {Movie.create(title: 'B', rating: '', release_date: '1936-05-05', description: 'this is a very interesting movie')}
    let(:movie5) {Movie.create(title: 'C', rating: 'G', release_date: '', description: 'this is a very interesting movie')}
    let(:movie6) {Movie.create(title: 'D', rating: 'G', release_date: '1936-05-05', description: '')}
    ...
    it 'should require a release_date' do
      expect(movie1).to be_valid
      expect(movie5).not_to be_valid
    end

    it 'should require a rating' do
      expect(movie1).to be_valid
      expect(movie4).not_to be_valid
    end

    it 'Availability and description length' do
      expect(movie1).to be_valid
      expect(movie6).not_to be_valid
